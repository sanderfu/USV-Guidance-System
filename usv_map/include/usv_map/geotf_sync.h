...
class GeodeticConverterSynchronized : private GeodeticConverter{
    public:
        GeodeticConverterSynchronized(std::string name, bool publish, bool subscribe,const ros::NodeHandle& nh);
        void addSyncedFrameByEPSG(std::string name, int code);
        void addSyncedFrameByENUOrigin(std::string name, double lat, double lon, double alt, bool replace=false);
